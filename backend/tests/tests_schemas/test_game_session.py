...
@pytest.mark.asyncio
async def test_next_question_normal_flow(session_empty_init):
    questions = [
...
@pytest.mark.asyncio
async def test_next_question_uses_generator_when_no_questions(session_empty_init):
    # ARRANGE
    session = session_empty_init
...
@pytest.mark.asyncio
async def test_next_question_generator_returns_none(session_empty_init):
    # ARRANGE
    session = session_empty_init
...
@pytest.mark.asyncio
async def test_next_question_reaches_max_limit(session_empty_init):
    # ARRANGE
    questions = [
...
@pytest.mark.asyncio
async def test_next_question_after_generation_continues_normally(session_empty_init):
    # ARRANGE
    questions = [
        Question(id=1, text="Q1", category=Category.RANDOM, language=Language.PL, answer=None)
    ]
